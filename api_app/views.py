...
# Get all the quotes from LoTR, requires authentication
@api_view(['GET'])
@authentication_classes([TokenAuthentication, SessionAuthentication])
@permission_classes([IsAuthenticated])
def get_quote_list(request):
    ...
    # Return the serialized data
    return JsonResponse({'quotes': serializer.data}, status=status.HTTP_200_OK)


# Get a quote by id, requires authentication
@api_view(['GET'])
@authentication_classes([TokenAuthentication, SessionAuthentication])
@permission_classes([IsAuthenticated])
def get_quote_by_id(request, quote_id):
    ...
    try:
        ...
        return JsonResponse({'message': 'The quote does not exist'}, status=status.HTTP_404_NOT_FOUND)

    # Serialize the data
    serializer = QuoteSerializer(quote)

    # Return the serialized data
    return JsonResponse({'quote': serializer.data}, status=status.HTTP_200_OK)